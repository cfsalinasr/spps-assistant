...
def _handle_reset(repository) -> None:
    """Prompt for confirmation and reset the database."""
...
def _handle_import(repository, import_path: str) -> None:
    """Import a materials library file into the database."""
...
def _handle_export(repository, export_path: str) -> None:
    """Export the residue library to a CSV file."""
...
def _handle_list(repository) -> None:
    """Display all residue records in a Rich table."""
...
def _handle_status(repository, db_path) -> None:
    """Display a brief database status panel."""
    records = repository.list_residues()
    console.print(Panel(
        f"[bold]Residue records:[/bold] {len(records)}\n"
        f"[bold]Location:[/bold] {db_path}\n\n"
...
def db(
    ...
    if reset:
        _handle_reset(repository)
        return

    if import_path:
        _handle_import(repository, import_path)

    if export_path:
        _handle_export(repository, export_path)

    if add_token:
        _add_single_residue(repository, add_token)

    if list_residues:
        _handle_list(repository)

    if not any([list_residues, export_path, import_path, reset, add_token]):
        _handle_status(repository, _DB_PATH)
